...
public:
  ...
  void replicate_task(MapperContext ctx, const Task& task, const
                      ReplicateTaskInput& input, ReplicateTaskOutput& output);
...
void PennantMapper::select_sharding_functor(const Mapping::MapperContext ctx,
                               const Task&                        task,
                               const SelectShardingFunctorInput&  input,
                                     SelectShardingFunctorOutput& output) {
...
void PennantMapper::select_task_options(MapperContext ctx, const Task &task, TaskOptions &output)
{
  ...
  if (!strcmp(task.get_task_name(), "wrapper") || !strcmp(task.get_task_name(), "toplevel"))
    ...
    void PennantMapper::replicate_task(MapperContext              ctx,
                                       const Task&                task,
...
void register_mappers()
{
  ...
  Runtime::preregister_sharding_functor(1, new LinearShardingFunctor());
}
