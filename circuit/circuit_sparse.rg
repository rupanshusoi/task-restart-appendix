...
task wire_copier(
    ...
    rs: region(node),
    rg: region(node),
    dst: region(wire(rp, rs, rg)),
    src: region(wire(rp, rs, rg))
    )
where writes(dst), reads(src) do
  for i in src do
    var { in_ptr, out_ptr, inductance, resistance, wire_cap, current, voltage } = src[i]
    dst[i] = [wire(rp, rs, rg)] { in_ptr, out_ptr, inductance, resistance, wire_cap, current, voltage }
  end
end
...
task wrapper(
            ...
            num_loops: int,
            num_restarts: int,
            launch_domain: ispace(ptr),
            all_times: region(timestamp),
            all_nodes: region(node),
            all_wires: region(wire(wild, wild, wild)),
            rp_times: partition(disjoint, all_times),
            rp_all_nodes: partition(disjoint, all_nodes),
            all_private: region(node),
            rp_private: partition(disjoint, all_private),
            all_shared: region(node),
            rp_shared: partition(disjoint, all_shared),
            rp_wires: partition(disjoint, all_wires),
            rp_ghost: partition(aliased, all_shared)
            )
where reads writes(all_times, all_nodes, all_wires), all_private <= all_nodes, all_shared <= all_nodes
do
  var all_times_cpy = region(all_times.ispace, timestamp)
  var rp_times_cpy_raw = c.legion_logical_partition_create(
      __runtime(), __raw(all_times_cpy), __raw(rp_times).index_partition)
  var rp_times_cpy = __import_partition(disjoint, all_times_cpy, rp_times.colors, rp_times_cpy_raw)
  __demand(__index_launch)
  for i in rp_times.colors do
    [make_copier(region(timestamp), region(timestamp))]
      (rp_times_cpy[i], rp_times[i])
  end

  var all_wires_cpy = region(all_wires.ispace, wire(wild, wild, wild))
  var rp_wires_cpy_raw = c.legion_logical_partition_create(
      __runtime(), __raw(all_wires_cpy), __raw(rp_wires).index_partition)
  var rp_wires_cpy = __import_partition(disjoint, all_wires_cpy, rp_wires.colors, rp_wires_cpy_raw)
  __demand(__index_launch)
  for i in rp_wires_cpy.colors do
    wire_copier(all_nodes, all_nodes, all_nodes, rp_wires_cpy[i], rp_wires[i])
  end

  var all_nodes_cpy = region(all_nodes.ispace, node)
  var rp_all_nodes_cpy_raw = c.legion_logical_partition_create(
      __runtime(), __raw(all_nodes_cpy), __raw(rp_all_nodes).index_partition)
  var rp_all_nodes_cpy = __import_partition(disjoint, all_nodes_cpy, rp_all_nodes.colors, rp_all_nodes_cpy_raw)

  var all_private_cpy = rp_all_nodes_cpy[0]
  var all_shared_cpy = rp_all_nodes_cpy[1]

  var rp_private_cpy_raw = c.legion_logical_partition_create(
      __runtime(), __raw(all_private_cpy), __raw(rp_private).index_partition)
  var rp_private_cpy = __import_partition(disjoint, all_private_cpy, rp_private.colors, rp_private_cpy_raw)

  var rp_shared_cpy_raw = c.legion_logical_partition_create(
      __runtime(), __raw(all_shared_cpy), __raw(rp_shared).index_partition)
  var rp_shared_cpy = __import_partition(disjoint, all_shared_cpy, rp_shared.colors, rp_shared_cpy_raw)

  __demand(__index_launch)
  for i in rp_private.colors do
    [make_copier(region(node), region(node))]
      (rp_private_cpy[i], rp_private[i])
  end

  __demand(__index_launch)
  for i in rp_shared.colors do
    [make_copier(region(node), region(node))]
      (rp_shared_cpy[i], rp_shared[i])
  end
  ...
  while true do
    ...
    for i in rp_times.colors do
      [make_copier(region(timestamp), region(timestamp))]
        (rp_times[i], rp_times_cpy[i])
    end

    __demand(__index_launch)
    for i in rp_wires_cpy.colors do
      wire_copier(all_nodes, all_nodes, all_nodes, rp_wires[i], rp_wires_cpy[i])
    end

    __demand(__index_launch)
    for i in rp_private.colors do
      [make_copier(region(node), region(node))]
        (rp_private[i], rp_private_cpy[i])
    end

    __demand(__index_launch)
    for i in rp_shared.colors do
      [make_copier(region(node), region(node))]
        (rp_shared[i], rp_shared_cpy[i])
    end
  end
end
...
task print_my_time(time: int64)
  format.println("My elapsed time: {7.3}", time * 1e-6)
end
  ...
  rescape
    ...
    if en == "ON" then
      ...
      remit rquote
        ...
        regentlib.assert(unroll <= num_loops, "LEGION_RESTART_UNROLL must be <= num_loops")
        regentlib.assert(num_loops % unroll == 0, "LEGION_RESTART_UNROLL does not evenly divide num_loops")
        var step : int = num_loops / unroll
        var num_restarts = 0
        __fence(__execution, __block)
        var my_start_time = get_time()
        for i = 0, step do
        ...
        __fence(__execution, __block)
        var my_end_time = get_time()
        print_my_time(my_end_time - my_start_time)
      end 
    elseif en == "OFF" then
      remit rquote
        __fence(__execution, __block)
