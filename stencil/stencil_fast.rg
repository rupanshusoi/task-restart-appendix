...
local function make_copier(dst_ty, src_ty)
  ...
  local task copier([dst], [src]) where writes(dst), reads(src), src * dst do
    for i in src do
      dst[i] = src[i]
    end
  end
...
task inner_wrapper(
  ...
  interior: partition(disjoint, points, tiles),
  exterior: partition(disjoint, points, tiles),
  times: region(ispace(int1d), timestamp),
  p_times: partition(disjoint, times, ispace(int1d)),
  xm: region(ispace(int2d), point),
  pxm_in: partition(disjoint, xm, tiles),
  pxm_out: partition(disjoint, xm, tiles),
  xp: region(ispace(int2d), point),
  pxp_in: partition(disjoint, xp, tiles),
  pxp_out: partition(disjoint, xp, tiles),
  ym: region(ispace(int2d), point),
  pym_in: partition(disjoint, ym, tiles),
  pym_out: partition(disjoint, ym, tiles),
  yp: region(ispace(int2d), point),
  pyp_in: partition(disjoint, yp, tiles),
  pyp_out: partition(disjoint, yp, tiles)
  )
where reads writes(points, times, xm, xp, ym, yp) do
  __demand(__spmd)
  do
    -- for i = 0, nt2 do
...
where reads writes(points, times, xm, xp, ym, yp) do
  var points_cpy = region(points.ispace, point)
  var private_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(points_cpy), __raw(private).index_partition)
  var private_cpy = __import_partition(disjoint, points_cpy, private.colors, private_cpy_raw)
  __demand(__index_launch)
  for i in private.colors do
    [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
      (private_cpy[i], private[i])
  end

  var times_cpy = region(times.ispace, timestamp)
  var p_times_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(times_cpy), __raw(p_times).index_partition)
  var p_times_cpy = __import_partition(disjoint, times_cpy, p_times.colors, p_times_cpy_raw)
  __demand(__index_launch)
  for i in p_times.colors do
    [make_copier(region(ispace(int1d), timestamp), region(ispace(int1d), timestamp))]
      (p_times_cpy[i], p_times[i])
  end

  var xm_cpy = region(xm.ispace, point)
  var pxm_out_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(xm_cpy), __raw(pxm_out).index_partition)
  var pxm_out_cpy = __import_partition(disjoint, xm_cpy, pxm_out.colors, pxm_out_cpy_raw)
  __demand(__index_launch)
  for i in pxm_out.colors do
    [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
      (pxm_out_cpy[i], pxm_out[i])
  end

  var xp_cpy = region(xp.ispace, point)
  var pxp_out_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(xp_cpy), __raw(pxp_out).index_partition)
  var pxp_out_cpy = __import_partition(disjoint, xp_cpy, pxp_out.colors, pxp_out_cpy_raw)
  __demand(__index_launch)
  for i in pxp_out.colors do
    [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
      (pxp_out_cpy[i], pxp_out[i])
  end

  var ym_cpy = region(ym.ispace, point)
  var pym_out_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(ym_cpy), __raw(pym_out).index_partition)
  var pym_out_cpy = __import_partition(disjoint, ym_cpy, pym_out.colors, pym_out_cpy_raw)
  __demand(__index_launch)
  for i in pym_out.colors do
    [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
      (pym_out_cpy[i], pym_out[i])
  end

  var yp_cpy = region(yp.ispace, point)
  var pyp_out_cpy_raw = c.legion_logical_partition_create(__runtime(), __raw(yp_cpy), __raw(pyp_out).index_partition)
  var pyp_out_cpy = __import_partition(disjoint, yp_cpy, pyp_out.colors, pyp_out_cpy_raw)
  __demand(__index_launch)
  for i in pyp_out.colors do
    [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
      (pyp_out_cpy[i], pyp_out[i])
  end
  ...
  while true do
    ...
    for i in private.colors do
      [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
        (private[i], private_cpy[i])
    end

    __demand(__index_launch)
    for i in p_times.colors do
      [make_copier(region(ispace(int1d), timestamp), region(ispace(int1d), timestamp))]
        (p_times[i], p_times_cpy[i])
    end

    __demand(__index_launch)
    for i in pxm_out.colors do
      [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
        (pxm_out[i], pxm_out_cpy[i])
    end

    __demand(__index_launch)
    for i in pxp_out.colors do
      [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
        (pxp_out[i], pxp_out_cpy[i])
    end

    __demand(__index_launch)
    for i in pym_out.colors do
      [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
        (pym_out[i], pym_out_cpy[i])
    end

    __demand(__index_launch)
    for i in pyp_out.colors do
      [make_copier(region(ispace(int2d), point), region(ispace(int2d), point))]
        (pyp_out[i], pyp_out_cpy[i])
    end
...
task main()
  ...
  var private = make_private_partition(points, tiles, n, nt, radius) -- complete
  ...
  var pxm_out = [make_ghost_x_partition(true)](xm, tiles, n, nt, radius, 0) -- complete
  var pxp_out = [make_ghost_x_partition(true)](xp, tiles, n, nt, radius, 0) -- complete
  var pym_out = [make_ghost_y_partition(true)](ym, tiles, n, nt, radius, 0) -- complete
  var pyp_out = [make_ghost_y_partition(true)](yp, tiles, n, nt, radius, 0) -- complete
  ...
  rescape
    ...
    if en == "ON" then
      ...
      remit rquote
        var unroll = [_unroll]
        regentlib.assert(unroll <= tsteps, "LEGION_RESTART_UNROLL must be <= tsteps")
        regentlib.assert(tsteps % unroll == 0, "LEGION_RESTART_UNROLL does not evenly divide tsteps")
        var step = tsteps / unroll
        var num_restarts = 0
        for i = 0, step do
          if i == 0 then num_restarts = 1 else num_restarts = 0 end
          wrapper(
            tsteps,
            tprune,
            init,
    ...
    elseif en == "OFF" then
      ...
      assert(false, "unknown value for LEGION_RESTART, expected ON or OFF")
    end
  end
